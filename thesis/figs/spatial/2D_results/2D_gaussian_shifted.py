...
class ConnectLayers_tester(object):
    ...
    def ks_test(self, control=False, alternative='two_sided',
        ...
        if show_network and not control:
            ...
            topo.PlotLayer(self.lt, nodesize=nodesize, fig=fig,
                                 nodecolor='grey')
            ...
            plt.scatter((0.25), (0.25), marker='.', s=70, color='k')
            from matplotlib.lines import Line2D
            l1 = Line2D([-0.25,-0.25], [-0.25,-0.57], linestyle='dashed', color='purple', 
                        linewidth=3, zorder=-2)
            l2 = Line2D([-0.25,-0.57], [-0.25,-0.25], linestyle='dashed', color='purple', 
                        linewidth=3, zorder=-1)
            ax.add_line(l1)
            ax.add_line(l2)
            ax.arrow(-0.35, 0.25, -0.1, 0.0, shape='full', head_width=0.03, head_length=0.03, 
                     fc="k", ec="k", length_includes_head="True", linewidth=3)
            ax.arrow(0.25, -0.35, 0.0, -0.1, shape='full', head_width=0.03, head_length=0.03, 
                     fc="k", ec="k", length_includes_head="True", linewidth=3)
            ax.arrow(-0.35, -0.35, -0.1, -0.1, shape='full', head_width=0.03, head_length=0.03, 
                     fc="k", ec="k", length_includes_head="True", linewidth=3)
        ...
        if show_PDF:
            ...
            theory = plt.plot(x, y, color='black', linewidth=3,
                     label='$\mathrm{Theory}$', zorder=1)
...
if __name__ == "__main__":
    ...
    f.write(str(test.N) + ', ' + str(msd) + ', ' + str(ks) + ', ' + str(p) + '\n')
    f.close()
