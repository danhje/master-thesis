...
class ConnectLayers_tester(object):
    ...
    def _pdf(self, D):
        ...
        if D <= self.L / 2.:
            ...
            return max(0., (min(1., self.kernel_func(D))) * D *
    ...
    def ks_test(self, control=False, bias=None, show_network=False,
                show_CDF=False, show_PDF=False, histogram_bins=100):
        '''
    ...
    def z_test(self, control=False, bias=None):
        ...
        dist = topo.Distance(self.driver, nest.GetLeaves(self.lt)[0])
        ps = ([self.kernel_func(D) for D in dist])
        exp = sum(ps)
        var = sum([p * (1 - p) for p in ps])
        sd = np.sqrt(var)
        z = abs((num - exp) / sd)
        p2 = 2 * (1 - scipy.stats.norm.cdf(z))
...
def run(bias, ranges=None):
    if ranges is None:
        ranges = ([10] + range(2000, 10001, 2000) + [15000] + range(20000, 100001, 10000) + 
