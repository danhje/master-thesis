...
if __name__ == "__main__":
    ...
    fig = plt.figure(1, figsize=(6.0, 5.0))
    f = open('RCC_distinct.txt', 'a')

    plt.subplot(2, 2, 1)
    test = RCC_tester(N_s=10, N_t=5, C=10, e_min=1)
    ...
    plt.subplot(2, 2, 2)
    test = RCC_tester(N_s=10, N_t=10, C=5, e_min=1)
    ks, p = test.ks_test(n=10000, start_seed=0, control=True)
    print 'p-value of KS-test of uniformity:', p
    f.write(str(ks) + ', ' + str(p) + '\n')
    test.pvalues.sort()
    ...
    a.set_xticklabels(a.get_xticks(), fontprop)
    a.set_yticklabels(a.get_yticks(), fontprop)
    plt.title('C', {'fontsize': 14, 'ha': 'center'})

    plt.subplot(2, 2, 4)
    test = RCC_tester(N_s=5, N_t=10, C=10)
    ks, p = test.ks_test(n=10000, start_seed=0, control=True)
    print 'p-value of KS-test of uniformity:', p
    f.write(str(ks) + ', ' + str(p) + '\n')
    test.pvalues.sort()
    ...
    f.close()
    plt.tight_layout()
    plt.savefig('RCC_distinct.pdf', bbox_inches='tight')
    plt.cla()
    plt.close('all')
