...
class RCC_tester(object):
    ...
    def ks_test(self, n, seed_start=None, control=False,
...
if __name__ == "__main__":
    ...
    ppp = []
    for seed_start in range(0, 20000*100+1, 20000):
        print '='*20
        print '='*20
        print 'Running super-test %d of %d.' % (1+seed_start/20000, 100)
        print '='*20
        print '='*20
        e = RCC_tester(N_s=1000, N_t=1000, C=1000, e_min=10)
        ks, p = e.ks_test(n=1000, seed_start=seed_start, control=True)
        print 'p-value of KS-test of uniformity:', p
        ppp.append(p)

    filename_base = __file__.split('.')[0]

    numpy.savetxt(filename_base + '_pvs.txt', ppp)

    ks2, p2 = scipy.stats.kstest(ppp, 'uniform', alternative='two_sided')
    ...
    plt.step([0.0] + ppp + [1.0], [0.0] + y + [1.0], label='$S_n(x)$')
