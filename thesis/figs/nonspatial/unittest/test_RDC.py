...
class RDC_tester(object):
    '''
    Class used for testing RandomDivergentConnect.
    '''
    ...
    def __init__(self, N_s, N_t, C, e_min=10, threads=1):
        ...
        if expected_degree < self.e_min:
            raise RuntimeWarning(
                'Expected in-degree (%.2f) is less than e_min (%.2f).' \
                'Increase N_s*C / N_t or decrease e_min.' %
    ...
    def _connect(self):
        ...
        nest.RandomDivergentConnect(self.source_nodes, self.target_nodes,
    ...
    def _get_degrees(self, msd):
        ...
        target_connections = [conn[1] for conn in connections]
        degrees = self._counter(target_connections)
...
if __name__ == '__main__':
    test = RDC_tester(N_s=1000, N_t=1000, C=1000)
