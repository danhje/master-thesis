...
class RCC_tester(object):
    ...
    def ks_test(self, n, start_seed=None, control=False,
        ...
        if show_CDF:
            ...
            y = x
            plt.plot(x, y, '-', color='black', linewidth=3, label='$\mathrm{Theory}$')
            self.pvalues.sort()
            y = [(i + 1.) / len(self.pvalues)
                 for i in range(len(self.pvalues))]
            plt.step([0.0] + self.pvalues + [1.0], [0.0] + y + [1.0], color='red', label='$\mathrm{Observation}$')
            plt.xlim((0, 1))
            plt.ylim((0, 1))
            plt.xlabel('$x$', fontsize=20)
            plt.ylabel('$S_n(x)$', fontsize=20)
        ...
        if show_histogram:
            ...
            plt.xlabel('$\mathrm{p-values}$', fontsize=20)
...
if __name__ == "__main__":
    ...
    ks, p = test.ks_test(n=1000, start_seed=0, control=True, show_histogram=True,
                         histogram_bins=100, show_CDF=True)
