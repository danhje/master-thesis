...
class ConnectLayers_tester(object):
    ...
    def ks_test(self, control=False, alternative='two_sided',
        ...
        if show_CDF:
            plt.figure()
            plt.plot(dist, self.cdf_list, '-', color='grey', linewidth=10,
            ...
            plt.tight_layout()
            plt.savefig(filename_base + '_CDF.png', bbox_inches='tight')
        ...
        if show_PDF:
            ...
            theory = plt.plot(x, y, color='grey', linewidth=10,
                     label='$\mathrm{Theory}$', zorder=1)
            ...
            empirical = plt.plot([0], [0], 'r-', linewidth=3, label='$\mathrm{Observation}$', zorder=0)
            
            ax = plt.gca()
            handles, labels = ax.get_legend_handles_labels()
            
            #self.leg = plt.legend(handles[:-1], labels[:-1], loc='upper right')
            ...
            a.set_yticklabels(a.get_yticks(), fontprop)
            plt.tight_layout()
            plt.savefig(filename_base + '_PDF.png', bbox_inches='tight')
